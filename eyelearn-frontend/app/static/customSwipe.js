...
async function predict(imgElement) {
      ...
      console.log(topK[0].value);
      console.log(topK[1].value);
...
function addCards(classid, cat) {
    var imgs = document.getElementsByTagName("img");
    $("#tinderslide").remove()
    $('.wrap').prepend("<div id='tinderslide'><ul></ul></div>");
    $.ajax({
        url: '/getImages/' + classid + '/' + cat,
        type: 'GET',
        success: function (data) {
            console.log("here");
...
function startGame(loc, class_id, category, word) {
    modelLoc = loc;
    classid = class_id;
    cat = category
    answer = word;
